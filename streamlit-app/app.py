...
if monthly is not None:
    ...
    with col_left:
        ...
        st.altair_chart(top_stations_fig(monthly), use_container_width=True)
    with col_right:
        st.subheader("Top 10 Crime Types Near Stations")
        st.altair_chart(crime_type_fig(crime_types), use_container_width=True)

    st.divider()

    st.subheader("Total Crime vs. Total Ridership by Station")
    st.altair_chart(correlation_scatter_fig(monthly, LINE_COLORS), use_container_width=True)

    st.divider()

    st.subheader("Crime & Ridership Over Time")
    st.altair_chart(dual_axis_trend_fig(monthly), use_container_width=True)
    ...
    st.dataframe(corr_df, use_container_width=True, height=350)
